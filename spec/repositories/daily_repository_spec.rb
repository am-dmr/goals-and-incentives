...
describe DailyRepository do
  ...
  describe '#previous_day' do
    subject { described_class.new.previous_day(user) }
    ...
    let!(:dailies) do
      [
        create(:daily, goal: once, date: Date.yesterday),
        create(:daily, goal: per_day_xl, date: Date.current),
        create(:daily, goal: per_day_xl, date: Date.yesterday),
        create(:daily, goal: per_day_xs, date: Date.current),
        create(:daily, goal: per_day_xs, date: Date.yesterday),
        create(:daily, goal: per_week, date: Date.yesterday.beginning_of_week),
        create(:daily, goal: create(:goal), date: Date.yesterday)
      ]
    end
    ...
    it 'returns correct dailies in correct order' do
      expect(subject.pluck(:id)).to eq([dailies[2].id, dailies[4].id, dailies[5].id, dailies[0].id])
    end
  end
